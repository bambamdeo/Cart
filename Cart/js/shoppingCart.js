    ...
    if (items != null && JSON != null) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                if (item.sid != null && item.name != null && item.price != null && item.quantity != null) {
                    item = new cartItem(item.sid, item.name, item.price, item.quantity);
...
shoppingCart.prototype.addItem = function (sid, name, price, quantity) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(sid, name, price, quantity);
            this.items.push(item);
        }
...
shoppingCart.prototype.getTotalPrice = function (sid) {
    var total = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (sid == null || item.sid == sid) {
...
// get the total price for all items currently in the cart
shoppingCart.prototype.getTotalCount = function (sid) {
    var count = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (sid == null || item.sid == sid) {
...
shoppingCart.prototype.checkoutPayPal = function (parms, clearCart) {
    ...
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        var ctr = i + 1;
        data["item_number_" + ctr] = item.sid;
...
// items in the cart
//
function cartItem(sid, name, price, quantity) {
    this.sid = sid;
    this.name = name;
    this.price = price * 1;
